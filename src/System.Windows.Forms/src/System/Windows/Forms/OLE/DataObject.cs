...
#pragma warning restore CS3001 // Argument type is not CLS-compliant
    ...
    public virtual Stream? GetAudioStream() =>
        ...
        GetDataInternal<Stream>(DataFormats.WaveAudio, autoConvert: false);

    public virtual StringCollection GetFileDropList()
    {
        StringCollection dropList = [];
        string[]? strings = GetDataInternal<string[]>(DataFormats.WaveAudio, autoConvert: false);
    ...
    public virtual Image? GetImage() => GetDataInternal<Bitmap>(DataFormats.Bitmap, autoConvert: true);
    ...
    private T? GetDataInternal<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(string format, bool autoConvert)
    {
    ...
    internal static bool ValidateTryGetDataArguments<T>(string format)
    {
        ...
        if (!IsRestrictedFormat(format)
            // check is a convenience for simple usages where we aren't passing a resolver explicitly.
            && (type == typeof(object) || type.IsInterface || type.IsAbstract))
        {
            // TODO: localize string
            throw new NotSupportedException(
