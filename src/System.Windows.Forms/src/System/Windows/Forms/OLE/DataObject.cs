    ...
    Com.IManagedWrapper<Com.IDataObject>
{
    ...
    public virtual T? GetData<T>(string format, bool autoConvert) where T : class =>
        ((IDataObject)_innerData).GetData<T>(format, autoConvert);

    public virtual T? GetData<T>(string format) where T : class => GetData<T>(format, autoConvert: true);

    public virtual T? GetData<T>() where T : class => GetData<T>(typeof(T).FullName!);
