...
public static class Clipboard
{
    ...
    /// <devdoc>
    ///  This resolver is used in deserialization of Clipboard content and when wrapped into a serialization binder, passes all types through.
    ///  It is used in compatibility scenarios when user explicitly opts into its use.
    /// </devdoc>
    // Nullability warning is suppressed because the application had explicitly opted into unsafe compatible behavior.
    internal static Type UnboundedResolver(TypeName typeName) => null!;

    internal static Func<TypeName, Type> GetDataResolver() =>
        Switches.ClipboardEnableUnsafeBinaryFormatterDeserialization ? UnboundedResolver : NotSupportedResolver;
    ...
    {
        return GetDataObjectInternal(legacyCompatMode: false);
    }

    private static unsafe IDataObject? GetDataObjectInternal(bool legacyCompatMode)
    {
        if (Application.OleRequired() != ApartmentState.STA)
        {
        ...
        if (managedDataObject is not Com.IDataObject.Interface dataObject)
        {
            ...
            return new DataObject(proxyDataObject.Value, legacyCompatMode);
        }
        ...
        return dataObject is IDataObject ido && !Marshal.IsComObject(dataObject)
            ? ido
            : new DataObject(dataObject, legacyCompatMode);
    }
    ...
    public static Stream? GetAudioStream() =>
        GetDataInternal<Stream>(DataFormats.WaveAudioConstant, autoConvert: false);
    ...
    /// </exception>
    /// <exception cref="NotSupportedException">This value requires <see cref="BinaryFormatter"/> deserialization and
    ///  the following 2 conditions are not satisfied: application opt-ins into <see cref="BinaryFormatter"/> use, and
    ///  application opts-in into <see cref="BinaryFormatter"/> deserialization in the WinForms Clipboard.
    /// </exception>
    public static object? GetData(string format) =>
        string.IsNullOrWhiteSpace(format) ? null : GetDataInternal<object>(format, autoConvert: false);

    private static T? GetDataInternal<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(string format, bool autoConvert)
    {
        if (GetDataObjectInternal(legacyCompatMode: true) is IDataObject dataObject)
        {
            dataObject.TryGetData(format, GetDataResolver(), autoConvert, out T? data);
            return data;
        }
    ...
    {
        data = default;
        if (GetDataObjectInternal(legacyCompatMode: false) is IDataObject dataObject)
        {
            ...
            return dataObject.TryGetData(format, NotSupportedResolver, autoConvert: false, out data);
        }
...
#pragma warning restore CS3001 // Argument type is not CLS-compliant
    ...
    public static StringCollection GetFileDropList()
    {
        ...
        if (GetDataInternal<string[]>(DataFormats.FileDropConstant, autoConvert: true) is string[] strings)
        {
    ...
    /// </summary>
    /// <devdoc>
    ///    <see cref="Bitmap"/>s are resolved by the BinaryFormattedObject but if that fails,
    ///    <see cref="BinaryFormatter"/> is restricted by the BitmapBinder.
    /// </devdoc>
    public static Image? GetImage() =>
        // Serialization and deserialization of DataFormats.Bitmap is restricted to Bitmap type.
        GetDataInternal<Bitmap>(DataFormats.Bitmap, autoConvert: true);
    ...
    {
        SourceGenerated.EnumValidator.Validate(format, nameof(format));
        if (GetDataInternal<string>(ConvertToDataFormats(format), autoConvert: false) is { } data)
        {
            return data;
        }
    ...
    public static void SetDataAsJson<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(T data) =>
        SetData(typeof(T).FullName!, data!); // TanyaSo SetDataAsJson(typeof(T).FullName!, data)
    ...
    [ThreadStatic]
    private static RequestedType t_typeFromLegacyCall;
    private class RequestedTypeScope : IDisposable
    {
        public RequestedTypeScope(RequestedType requestedType) => t_typeFromLegacyCall = requestedType;
        public void Dispose() => t_typeFromLegacyCall = RequestedType.None;
    }

    private enum RequestedType
    {
        None = 0,
        String = 1,
        StringArray = 2,
