...
public static class Clipboard
{
    ...
    internal static Type CompatibleResolver(TypeName typeName) => null!;

    internal static Func<TypeName, Type> Resolver { get; } =
        Switches.ClipboardEnableUnsafeBinaryFormatterDeserialization ? CompatibleResolver : NotSupportedResolver;
    ...
    {
        TryGetDataInternal(DataFormats.WaveAudioConstant, Resolver, autoConvert: false, out Stream? data);
        return data;
    }
    ...
    private static object? GetData(string format, bool autoConvert)
    {
        ...
        {
            dataObject.TryGetData(format, Resolver, autoConvert, out object? data);
            return data;
        }
    ...
    /// <exception cref="NotSupportedException">If application does not support BinaryFormatter and the object can't be deserialized otherwise.</exception>
...
#pragma warning restore CS3001 // Argument type is not CLS-compliant
    ...
    public static StringCollection GetFileDropList()
    {
        ...
        if (TryGetDataInternal(DataFormats.FileDropConstant, Resolver, autoConvert: true, out string[]? strings))
        {
    ...
    /// <remarks>
    ///  <para>
    ///    This method is guaranteed safe as it Images are resolved by the BinaryFormattedObject but if that fails,
    ///    BinaryFormatter is restricted by the BitmapBinder.
    ///  </para>
    /// </remarks>
    public static Image? GetImage()
    {
        TryGetDataInternal(DataFormats.Bitmap, Resolver, autoConvert: true, out Image? image);
        return image;
    }
    ...
    {
        SourceGenerated.EnumValidator.Validate(format, nameof(format));
        if (TryGetDataInternal(ConvertToDataFormats(format), Resolver, autoConvert: false, out string? data))
        {
            return data;
        }
