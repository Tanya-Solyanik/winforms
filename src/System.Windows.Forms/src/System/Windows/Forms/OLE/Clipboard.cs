...
public static class Clipboard
{
    ...
    {
        if (!DataObject.ValidateGetDataArguments<T>(format))
        {
            return default;
        }

        if (GetDataObject() is { } dataObject)
        {
...
#pragma warning restore CS3001 // Argument type is not CLS-compliant
    ...
    {
        data = default;
        if (!DataObject.ValidateTryGetDataResolverArguments<T>(format, resolver))
        {
            return false;
        }

        return GetDataObject() is { } dataObject && dataObject.TryGetData(format, resolver, autoConvert: false, out data);
    }
    ...
    {
        data = default;
        if (!DataObject.ValidateTryGetDataArguments<T>(format))
        {
            return false;
        }

        return GetDataObject() is { } dataObject && dataObject.TryGetData(format, NotSupportedResolver, autoConvert, out data);
    }
