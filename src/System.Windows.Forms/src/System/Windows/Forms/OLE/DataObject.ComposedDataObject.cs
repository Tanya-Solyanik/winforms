...
public unsafe partial class DataObject
{
    ...
    internal unsafe partial class ComposedDataObject : IDataObject, Com.IDataObject.Interface, ComTypes.IDataObject
    {
        ...
        bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(string format, Func<TypeName, Type> resolver, bool autoConvert, [NotNullWhen(true)] out T? data) where T : default
        {
            data = default;
            return _winFormsDataObject.TryGetData(format, resolver, autoConvert, out data);
        }

        bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(string format, bool autoConvert, [NotNullWhen(true)] out T? data) where T : default
        {
            data = default;
            return _winFormsDataObject.TryGetData(format, autoConvert, out data);
        }

        bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(string format, [NotNullWhen(true)] out T? data) where T : default
        {
            data = default;
            return _winFormsDataObject.TryGetData(format, out data);
        }

        bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>([NotNullWhen(true)] out T? data) where T : default
        {
            data = default;
