        ...
        private unsafe class NativeDataObjectToWinFormsAdapter : IDataObject, Com.IDataObject.Interface
        {
            ...
            private static bool TryGetDataFromHGLOBAL<T>(HGLOBAL hglobal, string format, Func<TypeName, Type> resolver, out T? data)
            {
                ...
                object? value = format switch
                {
                    ...
                    _ => ReadObjectOrStreamFromHGLOBAL<T>(hglobal, resolver, RestrictDeserializationToSafeTypes(format))
                };
            ...
            private static object? ReadObjectOrStreamFromHGLOBAL<T>(HGLOBAL hglobal, Func<TypeName, Type> resolver, bool restrictDeserialization)
            {
                ...
                if (!binaryFormattedObject.Contains<T>() && !RootIsAssignableTo<T>(binaryFormattedObject, resolver))
                {
                    return default;
                }
...
#pragma warning restore SYSLIB0011
            ...
            private static bool TryGetHGLOBALData<T>(Com.IDataObject* dataObject, string format, Func<TypeName, Type> resolver, out bool doNotContinue, out T? data)
            {
                data = default;
                doNotContinue = false;
            ...
            private static unsafe bool TryGetIStreamData<T>(Com.IDataObject* dataObject, string format, Func<TypeName, Type> resolver, out T? data)
            {
                data = default;
                Com.FORMATETC formatEtc = new()
                {
            ...
            private bool ValidateAndTryGetDataInternal<T>(string format, bool autoConvert, out T? data)
            {
                ...
                return TryGetDataInternal(format, Clipboard.NotSupportedResolver, autoConvert, out data);
            }
            ...
            private bool TryGetDataInternal<T>(string format, Func<TypeName, Type> resolver, bool autoConvert, out T? data)
            {
                ...
                bool result = TryGetObjectFromDataObject(nativeDataObject, format, resolver!, out bool doNotContinue, out data);
                ...
                foreach (string mappedFormat in mappedFormats)
                {
                    ...
                    result = TryGetObjectFromDataObject(nativeDataObject, mappedFormat, resolver, out doNotContinue, out data);
                    if (doNotContinue)
                    {
