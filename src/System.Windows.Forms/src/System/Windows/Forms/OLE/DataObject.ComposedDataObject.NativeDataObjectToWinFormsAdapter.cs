        ...
        private unsafe class NativeDataObjectToWinFormsAdapter : IDataObject, Com.IDataObject.Interface
        {
            ...
            private static object? ReadObjectOrStreamFromHGLOBAL<T>(HGLOBAL hglobal, Func<TypeName, Type> resolver, bool restrictDeserialization, bool legacyCompatMode)
            {
                ...
                catch (Exception ex) when (!ex.IsCriticalException())
                {
                    ...
                    // BinaryFormattedObject can't handle IObjectReference graphs, let BinaryFormatter try to deserialize the stream.
                    if (legacyCompatMode)
                    {
...
#pragma warning restore SYSLIB0011
            ...
            bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(
                ...
                bool autoConvert,
                [NotNullWhen(true), MaybeNullWhen(false)] out T data) =>
                TryGetDataInternal(format, Clipboard.NotSupportedResolver, autoConvert, out data);

            bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(
                string format,
                [NotNullWhen(true), MaybeNullWhen(false)] out T data) =>
                TryGetDataInternal(format, Clipboard.NotSupportedResolver, autoConvert: false, out data);

            bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(
                [NotNullWhen(true), MaybeNullWhen(false)] out T data) =>
                TryGetDataInternal(typeof(T).FullName!, Clipboard.NotSupportedResolver, autoConvert: false, out data);
