...
public unsafe partial class DataObject
{
    internal unsafe partial class ComposedDataObject
    {
        ...
        /// </summary>
        private unsafe partial class NativeDataObjectToWinFormsAdapter : IDataObject, Com.IDataObject.Interface
        {
            private readonly AgileComPointer<Com.IDataObject> _nativeDataObject;
            private readonly bool _legacy;

            public NativeDataObjectToWinFormsAdapter(Com.IDataObject* dataObject, bool legacy = false)
            {
                _legacy = legacy;
#if DEBUG
            ...
            /// </summary>
            private static bool TryGetDataFromHGLOBAL<T>(HGLOBAL hglobal, string format, Func<TypeName, Type> resolver, bool legacyCompatMode, out T? data)
            {
                data = default;
                if (hglobal == 0)
                {
                ...
                object? value = format switch
                {
                    ...
                    _ => ReadObjectOrStreamFromHGLOBAL<T>(hglobal, resolver, RestrictDeserializationToSafeTypes(format), legacyCompatMode)
                };
            ...
            private static object? ReadObjectOrStreamFromHGLOBAL<T>(HGLOBAL hglobal, Func<TypeName, Type> resolver, bool restrictDeserialization, bool legacyCompatMode)
            {
                ...
                catch (Exception ex) when (!ex.IsCriticalException())
                {
                    ...
                    // BinaryFormattedObject can't handle IObjectReference graphs, or offsets arrays, let BinaryFormatter try to deserialize the stream.
                    if (legacyCompatMode)
                    {
                ...
                if (!legacyCompatMode && !binaryFormattedObject.Contains<T>() && !RootIsAssignableTo<T>(binaryFormattedObject, resolver))
                {
                    return default;
                }
...
#pragma warning restore SYSLIB0011
            ...
            private static bool RootIsAssignableTo<T>(BinaryFormattedObject binaryFormattedObject, Func<TypeName, Type> resolver)
            {
                if (resolver is null)
                {
                    return false;
                ...
                return (type is not null) && type.IsAssignableTo(typeof(T));
            }
            ...
            ///   </para>
            /// </remarks>
            private static bool TryGetObjectFromDataObject<T>(Com.IDataObject* dataObject, string format, Func<TypeName, Type> resolver, bool legacyCompatMode, out bool doNotContinue, out T? data)
            {
                data = default;
                doNotContinue = false;
                bool result = false;
                try
                {
                    ...
                    result = TryGetHGLOBALData(dataObject, format, resolver, legacyCompatMode, out doNotContinue, out data);
                    if (!result && !doNotContinue)
                    {
                        // Lastly check to see if the data is an IStream.
                        result = TryGetIStreamData(dataObject, format, resolver, legacyCompatMode, out data);
                    }
                }
                catch (BinaryFormattedObjectException ex)
                {
            ...
            private static bool TryGetHGLOBALData<T>(Com.IDataObject* dataObject, string format, Func<TypeName, Type> resolver, bool legacyCompatMode, out bool doNotContinue, out T? data)
            {
                data = default;
                doNotContinue = false;
                ...
                try
                {
                    ...
                    {
                        result = TryGetDataFromHGLOBAL(medium.hGlobal, format, resolver, legacyCompatMode, out data);
                    }
                }
            ...
            private static unsafe bool TryGetIStreamData<T>(Com.IDataObject* dataObject, string format, Func<TypeName, Type> resolver, bool legacyCompatMode, out T? data)
            {
                data = default;
                Com.FORMATETC formatEtc = new()
                {
                ...
                try
                {
                    ...
                    return TryGetDataFromHGLOBAL(hglobal, format, resolver, legacyCompatMode, out data);
                }
                finally
                {
                    if (!hglobal.IsNull)
            ...
            private bool TryGetDataInternal<T>(string format, Func<TypeName, Type> resolver, bool autoConvert, out T? data)
            {
                ...
                // resolver is required, thus we are not validating this parameter here.
                bool result = TryGetObjectFromDataObject(nativeDataObject, format, resolver, _legacy, out bool doNotContinue, out data);
                ...
                foreach (string mappedFormat in mappedFormats)
                {
                    ...
                    result = TryGetObjectFromDataObject(nativeDataObject, mappedFormat, resolver, _legacy, out doNotContinue, out data);
                    if (doNotContinue)
                    {
            ...
            {
                TryGetDataInternal(format, GetDataResolver(), autoConvert, out object? data);
                return data;
            }
            ...
            {
                if (!_legacy && !ValidateTryGetDataResolverArguments<T>(format, resolver))
                {
                    data = default;
                    return false;
                }
            ...
            bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(
                ...
                bool autoConvert,
                [NotNullWhen(true), MaybeNullWhen(false)] out T data) =>
                TryGetDataInternal(format, NotSupportedResolver, autoConvert, out data);

            bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(
                string format,
                [NotNullWhen(true), MaybeNullWhen(false)] out T data) =>
                TryGetDataInternal(format, NotSupportedResolver, autoConvert: false, out data);

            bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(
                [NotNullWhen(true), MaybeNullWhen(false)] out T data) =>
                TryGetDataInternal(typeof(T).FullName!, NotSupportedResolver, autoConvert: false, out data);
