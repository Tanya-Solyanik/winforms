...
#pragma warning restore SYSLIB0011
            ...
            bool IDataObject.TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(string format, Func<TypeName, Type> resolver, bool autoConvert, [NotNullWhen(true), MaybeNullWhen(false)] out T data)
            {
                using var nativeDataObject = _nativeDataObject.GetInterface();
                ...
                if (doNotContinue
                    || !autoConvert
                    || (data is not null && data is not MemoryStream) // TanyaSo todo if I have unicode text on the clipboard, would format == test give me a memory stream?
                    || GetMappedFormats(format) is not { } mappedFormats
            ...
            private static T? GetObjectFromDataObject<T>(Com.IDataObject* dataObject, string format, Func<TypeName, Type> resolver, out bool doNotContinue)
            {
                ...
                try
                {
                    if (typeof(T).IsAssignableTo(typeof(Bitmap)))
                    {
                ...
                T? ReadObjectFromHGLOBAL(HGLOBAL hglobal, bool restrictDeserialization)
                {
                    ...
                    T? ReadObjectFromStream(MemoryStream stream, bool restrictDeserialization)
                    {
                        ...
                        try
                        {
                            ...
                            // Try to read the known safe types.
                        ...
                        if (!EnableUnsafeBinaryFormatterInNativeObjectSerialization)
                        {
                            // Other failures result in returning false, this one is kept consistent with other use cases.
                            throw new NotSupportedException(SR.BinaryFormatterNotSupported);
                        }
