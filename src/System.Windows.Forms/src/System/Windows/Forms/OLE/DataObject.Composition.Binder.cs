    ...
    internal unsafe partial class Composition
    {
        internal sealed class Binder : SerializationBinder
        {
            ...
            public override Type? BindToType(string assemblyName, string typeName)
            {
                ...
                // We assume all built-in types are normalized to mscorlib assembly, as BinaryFormatter and
                // BinaryFormatterObject are doing so for compatibility with .NET Framework.
                ...
                if (_legacyMode)
                {
                    return Switches.ClipboardEnableUnsafeBinaryFormatterDeserialization
                        ? null
                        : throw new NotSupportedException($"Using BinaryFormatter is not supported in WinForms Clipboard" +
                            $"data deserialization. Can't resolve '{typeName}' from '{assemblyName}'.");
                }

                return _resolver(TypeName.Parse($"{typeName}, {assemblyName}"))
            ...
            {
                if (!TryGetForwardedFromName(type, out string? name))
                {
            ...
            private static bool TryGetForwardedFromName(Type type, [NotNullWhen(true)] out string? shortName)
            {
                shortName = null;
                ...
                {
                    shortName = ((TypeForwardedFromAttribute)first).AssemblyFullName.Split(',')[0].Trim();
                    return true;
                }
