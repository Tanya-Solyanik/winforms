...
#pragma warning disable CS3001 // Argument type is not CLS-compliant
    bool TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(string format, Func<TypeName, Type> resolver, bool autoConvert, [NotNullWhen(true)] out T? data)
#pragma warning restore CS3001 // Argument type is not CLS-compliant
    {
        if (GetData(format, autoConvert) is T t)
        {
            data = t;
            return true;
        }
    ...
    /// </summary>
    bool TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(string format, bool autoConvert, [NotNullWhen(true)] out T? data) =>
        TryGetData(format, resolver: ClipboardHelper.SafeResolver, autoConvert, out data);

    /// <summary>
    ///  Retrieves the data associated with the specified data format if that data is of type <typeparamref name="T"/>.
    /// </summary>
    bool TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(string format, [NotNullWhen(true)] out T? data) =>
    ...
    /// </summary>
    bool TryGetData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>([NotNullWhen(true)] out T? data) =>
