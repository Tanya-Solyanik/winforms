...
#pragma warning restore CS3001 // Argument type is not CLS-compliant
        ...
        [NotNullWhen(true), MaybeNullWhen(false)] out T data)
    {
        ...
        if (GetData(format, autoConvert: false) is T result)
        {
            data = result;
            return true;
        }
    ...
    {
        data = default;
        if (!DataObject.ValidateTryGetDataArguments<T>(typeof(T).FullName!))
        {
            return false;
        }

        if (GetData(typeof(T).FullName!, autoConvert: false) is T result)
        {
            data = result;
            return true;
        }
