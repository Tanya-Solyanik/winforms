...
internal static class WinFormsBinaryFormattedObjectExtensions
{
    ...
    public static TypeName GetRootTypeName(this BinaryFormattedObject format)
    {
        return format.RootRecord switch
        {
            ...
            IPrimitiveTypeRecord primitiveArray => primitiveArray is not ArrayRecord
                ? throw new NotSupportedException(" A root IPrimitiveTypeRecord is not an array.")
                : primitiveArray.PrimitiveType.GetTypeName(),
                ...
                case BinaryType.SystemClass:
                    {
                        return info is Type recordElementType
                            ...
                            : TypeName.Parse($"{(string)info!}, System.Runtime");
                    }
                ...
                case BinaryType.Class:
                    {
                        ...
                        if (info is Type recordElementType)
                        {
                            ...
                            elementAssemblyName = recordElementType.Assembly.FullName!.Split(',')[0].Trim();
                        }
                        else
                        {
    ...
    private static (string typeName, string assemblyName) GetTypeNameFromClassRecord(this BinaryFormattedObject format, ClassRecord record)
    {
        ...
        string assemblyName = (record.LibraryId.IsNull
            ? "System.Runtime"
            : ((BinaryLibrary)format[record.LibraryId]).LibraryName) ?? throw new NotSupportedException("Bad record.");
    ...
    public static bool Contains<T>(this BinaryFormattedObject format)
    {
        switch (format.RootRecord)
        {
            case ClassRecord record:
                {
                    ...
                    if (string.Equals(requestedTypeName, recordTypeName, StringComparison.Ordinal))
                    {
                        ...
                        if (TryGetForwardedFromName(requestedType, out string? name) && string.Equals(recordAssemblyName, name, StringComparison.Ordinal))
                        {
                            return true;
                        }
                    }
            ...
            case IPrimitiveTypeRecord primitiveArray:
                ...
                return primitiveArray.PrimitiveType.IsElementType<T>();

            case IBinaryArray binaryArray:
                return IsBinaryArrayOfType<T>(format, binaryArray);
            default:
    ...
    private static bool IsBinaryArrayOfType<T>(BinaryFormattedObject format, IBinaryArray binaryArray)
    {
        if (binaryArray is not ArrayRecord arrayRecord)
        {
            throw new NotSupportedException(" A root IBinaryArray is not an array record.");
        ...
        switch (arrayRecord.ElementType)
        {
            case BinaryType.SystemClass:
                {
                    string elementTypeName = info is Type recordElementType
                        ? recordElementType.FullName!
                        : (string)info!;

                    if (string.Equals(requestedTypeName, elementTypeName, StringComparison.Ordinal))
                    {
                        // DayOfWeek
                        if (string.Equals(requestedAssemblyName, "System.Runtime", StringComparison.Ordinal))
                        {
                            return true;
                        }
                    }
            ...
            case BinaryType.PrimitiveArray:
                return ((PrimitiveType)info!).IsElementType<T>();
            default:
                throw new NotSupportedException($"Unexpected BinaryArray type: {arrayRecord.ElementType}");
        }
    }
