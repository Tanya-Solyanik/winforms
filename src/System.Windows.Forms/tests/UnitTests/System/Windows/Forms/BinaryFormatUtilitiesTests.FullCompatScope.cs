...
public partial class BinaryFormatUtilitiesTests
{
    internal readonly ref struct FullCompatScope : IDisposable
    {
        ...
        {
            _binaryFormatterScope = new BinaryFormatterScope(enable: true);
            _binaryFormatterInClipboardDragDropScope = new BinaryFormatterInClipboardDragDropScope(enable: true);
            _nrbfSerializerInClipboardDragDropScope = new NrbfSerializerInClipboardDragDropScope(enable: false);
        }
