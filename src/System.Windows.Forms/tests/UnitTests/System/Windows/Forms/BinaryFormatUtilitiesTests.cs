...
public partial class BinaryFormatUtilitiesTests : IDisposable
{
    ...
    [Fact]
    public void RoundTripOfType_AsObject_Simple()
    {
        List<int> value = [1, 2, 3];
        RoundTripOfType<object>(value).Should().BeEquivalentTo(value);
    }

    [Fact]
    public void RoundTripOfType_RestrictedFormat_AsObject_Simple()
    {
        ...
        Action action = () => ReadObjectFromStream<object>(DataObject.NotSupportedResolver, restrictDeserialization: true);
        action.Should().Throw<Exception>(); // System.Windows.Forms.DataObject.RestrictedTypeDeserializationException
        using BinaryFormatterScope scope = new(enable: true);
        using BinaryFormatterInClipboardScope clipboardScope = new(enable: true);
    ...
    [Fact]
    public void RoundTripOfType_TestData_CustomResolver()
    {
